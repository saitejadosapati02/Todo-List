...
const loadListObject = () =>{
    ...
    parsedList.forEach(itemObj => {
        const newToDoItem = createNewItem(itemObj._id, itemObj_item);
...
const processSubmission = ()=> {
    const newEntryTest = getNewEntry();
    if(!newEntryTest.length) return;
    const nextItemId = calcNextItemId();
    const toDoItem = createNewItem(nextItemId, newEntryTest);
    toDoList.addItemToList(toDoItem);
    updatePersistentData(toDoList.getList());
    updateScreenReaderConfirmation(newEntryTest, "added");
    refreshThePage();
};
...
const updateScreenReaderConfirmation = (newEntryTest, actionVerb) => {
    document.getElementById("confirmation").textContent =`${newEntryTest} ${actionVerb}.`;
}